# Development Rules

## 기본 원칙

- Java 21을 기준으로 개발한다.
- 기본 거래 모드는 `PAPER_TRADING`이다.
- 실제 주문 API와 `REAL_TRADING`은 구현하지 않는다.
- 사용자가 웹 포트폴리오에서 명시 선택한 보유 PAPER 포지션 SELL만 거래 실행 UX 예외로 허용한다.
- 수동 BUY는 계속 금지한다.
- 민감 정보는 코드, 로그, 응답에 노출하지 않는다.
- 문서 갱신은 변경 범위에 따라 최소화한다.
- 테스트 성공 전에는 커밋하지 않는다.

## Git 관리

- 작업 시작 전 `git status --short --branch`로 현재 브랜치, ahead/behind 상태, 수정/추가 파일을 확인한다.
- 작업 트리가 더러운 상태일 수 있으므로 사용자가 만든 변경을 되돌리거나 덮어쓰지 않는다.
- `git reset --hard`, `git checkout -- <path>`, 강제 push 같은 파괴적 명령은 사용자 명시 승인 없이는 실행하지 않는다.
- 커밋 전 `git diff --stat`과 `git diff --cached --stat`으로 포함 범위를 확인한다.
- 커밋에는 현재 작업과 직접 관련된 파일만 stage한다.
- `.DS_Store`, 로컬 `.env`, 빌드 산출물, 임시 파일은 커밋하지 않는다.
- 테스트 실패 또는 보안 린트 실패 상태에서는 커밋하지 않고 실패 원인을 보고한다.

## 모듈 경계

한 클래스에 아래 책임을 함께 넣지 않는다.

- 시세 조회
- 전략 판단
- 전략 선택 Bean 구성
- 재진입 제한 조건
- 리스크 검증
- 주문 실행
- 포트폴리오 반영
- history 저장
- Telegram 처리
- Web 화면 표시와 사용자 입력 처리

Controller와 Scheduler는 비즈니스 로직을 직접 갖지 않는다.
React 컴포넌트도 비즈니스 판단, 리스크 검증, 주문 상태 전이를 직접 구현하지 않는다.

## 문서 갱신 기준

아래 변경은 관련 문서를 반드시 갱신한다.

- 전략 변경: `docs/trading/STRATEGY_POLICY.md`
- 리스크 변경: `docs/trading/RISK_POLICY.md`
- 주문 흐름 변경: `docs/trading/ORDER_LIFECYCLE.md`
- Telegram 변경: `docs/operations/TELEGRAM_UX.md`
- 장애 처리 변경: `docs/operations/RELIABILITY.md`
- WebSocket 시세 수신, REST fallback 변경: `docs/operations/RELIABILITY.md`
- 운영 명령, 환경변수, 실행 절차 변경: `docs/operations/OPERATIONS.md`
- 구조/모듈 경계 변경: `docs/harness/ARCHITECTURE.md`
- 보안 변경: `docs/harness/SECURITY_RULES.md`
- 매매 조건 변경과 운용 기록: `docs/trading/condition-records/`

아래 변경은 문서 갱신을 생략할 수 있다.

- 일반 UI 레이아웃/문구 개선
- 내부 refactor
- 테스트 보강
- 완료 기록만 늘어나는 변경

`PROJECT_HISTORY`, `HARNESS_STATUS`, `PROJECT_NEXT_STEPS`는 매 작업 필수 갱신 대상이 아니다.
다음 작업 자체가 바뀔 때만 `docs/project/PROJECT_NEXT_STEPS.md`를 갱신한다.

## 테스트 필수 변경

- 전략 판단
- 리스크 정책
- 주문 상태 흐름
- 포트폴리오 반영
- history 저장
- Telegram 명령/버튼
- scheduler 실행
- offset 저장소
- 보안 린트
- Web 거래 실행 UX
- Web 모니터링/analytics UX
- Scheduler 실행 주기와 중복 실행 방지
- WebSocket parser, reconnect, stale snapshot, REST fallback

## Web 규칙

- React 앱은 `frontend/`에 둔다.
- 웹은 기존 REST API를 호출하고, 서버 도메인 로직을 중복 구현하지 않는다.
- 웹은 조회 중심 운영 화면이며 임의 거래 실행 버튼을 추가하지 않는다.
- 예외적으로 선택 보유 포지션 PAPER SELL 버튼만 허용한다. 이 버튼은 서버의 PAPER 주문, 리스크, 포트폴리오, history 흐름을 호출해야 한다.
- 자동매매 켜기/끄기와 candidate scheduler 주기 변경은 `/api/scheduler/control`만 사용한다.
- 웹에서 실제 주문 API, `REAL_TRADING`, Upbit 인증 설정을 추가하지 않는다.
- analytics와 history 집계는 가능한 한 서버 API에서 수행한다.
- API 응답 타입이 바뀌면 frontend 타입과 관련 화면 테스트를 함께 수정한다.
- 프론트 변경 시 `frontend`에서 `npm run lint`, `npm run build`, `npm test`를 실행한다.
- 화면 구조, 반응형 레이아웃, 위험 액션 UX를 바꾸면 `frontend`에서 `npm run test:e2e`도 실행한다.
- Playwright 브라우저가 없는 새 환경에서는 1회 `cd frontend && npx playwright install chromium`을 실행한 뒤 `npm run test:e2e`를 사용한다.

## Web 금지사항

- API client 외부에서 `fetch`를 직접 호출하지 않는다.
- React 화면에서 후보 실행 API나 trading-flow 실행 API를 호출하지 않는다.
- React 화면에서 수동 BUY API를 호출하지 않는다.
- React 화면에서 허용되는 거래 실행 호출은 선택 보유 포지션 PAPER SELL API뿐이다.
- 허용되는 endpoint는 `POST /api/portfolio/positions/sell-selected`뿐이며, request body는 market 목록만 보낸다.
- `localStorage`와 `sessionStorage`에 거래, 알림, 토큰, 상태 값을 저장하지 않는다.
- 프론트 코드에 `REAL_TRADING` 문자열을 추가하지 않는다.
- 프론트 코드에 access key, secret, token, password, chat id 성격의 식별자를 추가하지 않는다.
- 프론트 코드에 real order endpoint를 추가하지 않는다.
- 실패한 실행 결과를 성공처럼 표시하지 않는다.

## Analytics 규칙

- React 화면에서 손익, 손절/익절, 반복 HOLD 사유를 임의 규칙으로 재계산하지 않는다.
- history 기반 집계가 필요하면 `analytics` 패키지에 API를 추가하고 서비스 테스트를 작성한다.
- analytics API 응답 필드를 바꾸면 frontend 타입과 화면 테스트를 함께 수정한다.
- 손실 원인 분석은 실제 주문 API 추가 없이 PAPER history 기준으로만 수행한다.

## Telegram 규칙

- Telegram 기본 동작은 조회 전용이다.
- `/run`, `/candidate-run`, 실행 callback은 `TELEGRAM_MANUAL_PAPER_EXECUTION_ENABLED=true`일 때만 PAPER 실행을 호출한다.
- 기본 inline menu에는 실행 버튼을 노출하지 않는다.
- Telegram 메시지는 현재 자동 실행 조건, 손익, 포트폴리오를 확인할 수 있어야 한다.
- Telegram은 실제 주문 API, `REAL_TRADING`, 거래소 인증 설정을 추가하지 않는다.

## Scheduler 금지사항

- `@Scheduled`에 `fixedRate`를 사용하지 않는다.
- 외부 API polling scheduler는 이전 호출이 끝나기 전 다음 호출이 겹치지 않도록 중복 실행 방지 장치를 둔다.
- 외부 API 실패로 애플리케이션이 종료되게 예외를 밖으로 흘리지 않는다.

## 작업 계획

- 완료 기록은 Git history를 source of truth로 본다.
- `docs/project/PROJECT_HISTORY.md`는 최근 맥락만 유지한다.
- 다음 작업은 `docs/project/PROJECT_NEXT_STEPS.md`에 하나만 기록한다.
- 새 기능 시작 전 `AGENTS.md`, `HARNESS_STATUS`, 필요한 active policy 문서만 확인한다.

## 세부 규칙

- 보안: `docs/harness/SECURITY_RULES.md`
- 운영: `docs/operations/OPERATIONS.md`
- Telegram: `docs/operations/TELEGRAM_UX.md`
- 전략: `docs/trading/STRATEGY_POLICY.md`
- 리스크: `docs/trading/RISK_POLICY.md`
