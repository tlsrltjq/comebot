...
- candidate scheduler: 설정된 market 후보를 PAPER 실행하고 선택 설정 시 요약 알림 발송 (1회 최대 BUY: `max-buys-per-run`)

## 현재 운영 범위

- 기본 거래 모드: `PAPER_TRADING`
- 실제 주문: 구현하지 않음
- 웹 역할: 조회 중심 운영 화면. 자동매매 제어와 선택 보유 PAPER 포지션 SELL만 허용
- Telegram 기본 역할: 조회 전용
- 자동 실행 대상: `ALL_KRW`
- legacy trading scheduler 기본 주기: 30초, 기본 disabled
- candidate scheduler 기본 주기: 60초, 기본 enabled
- exit scheduler 기본 주기: 1초, 기본 enabled
- 1회 PAPER 주문 금액: 10,000 KRW
- 익절/손절 기준: 1.5 / -0.7 (application.properties 기본값; 운영 `.env`는 2.0 / -1.5)
- 트레일링 스톱: 기본 비활성 (운영 `.env`: 활성)
- 같은 market 추가 진입: 기본 허용
- 허용된 웹 거래 실행 예외: 사용자가 명시 선택한 보유 PAPER 포지션 SELL
...
기본 전략은 `VolatilityBreakoutLongStrategy`(bean 이름)이며 내부 로직은 풀백 바운스 진입(ADR-010)으로 교체됐다. 후보 스캔 기반 롱 전용 PAPER 전략을 사용한다.
...
**진입 필터 (CandidateScannerService)**
- BTC 1h 트렌드 필터: `BtcTrendCacheService`가 EMA 기반으로 BTC 1h 추세를 판단. DOWN이면 모든 Upbit 후보 차단.
- 진행 중인 최신 캔들 제외: API 응답에서 완성되지 않은 마지막 캔들을 드롭한 뒤 판단.
- 거래소별 독립 필터: `strategy.candidate-scanner.upbit.*` / `.binance.*` 로 캔들 수·가격·거래대금 기준을 거래소별로 오버라이드.
- market 제외 목록: `market.selection.excluded-markets`로 특정 market을 스캔에서 제외.
- 기타: trend UP, lastCandleBullish, priceChangeRate, tradeAmountChangeRate, highLowRangeRate, distanceFromHigh, latestCandleTradeAmount.

candidate scheduler는 `max-buys-per-run`(기본 2)으로 1회 실행당 최대 BUY 수를 제한한다.
market 목록은 24시간 거래대금 내림차순이므로 상위 품질 후보가 먼저 처리된다.

모든 스캔 결과(SELECTED/SKIPPED 포함)는 `CandidateScanLogService`를 통해 `scanlog` 모듈에 기록한다.
`GET /api/candidate-scan-log?range=24h&exchange=UPBIT`로 조회한다.
...
트레일링 스톱은 `PositionExitSignalService`에서 포지션별 peak profit을 추적한다. `risk.trailing-stop-activation-rate` 이상 수익 달성 후 `risk.trailing-stop-trail-rate` 이상 하락하면 SELL 신호를 만든다. 기본값은 비활성(`risk.trailing-stop-enabled=false`). 비정상 급락 시세 감지(`risk.abnormal-exit-price-drop-rate`, 기본 -20%) 시에는 자동 SELL을 차단한다.
...
웹은 조회 중심 운영 화면이다. 자동매매 상태, 후보, 포트폴리오, history, analytics를 표시하고, 자동매매 켜기/끄기와 candidate scheduler 주기 변경만 `/api/scheduler/control`로 수행한다.

포트폴리오 화면은 선택 거래소 기준으로 현금/포지션/market/거래소 비중을 원형 그래프와 legend로 표시한다. `UPBIT`과 `BINANCE` 자산은 환산 없이 각각의 기준 통화(`KRW`, `USDT`) 안에서만 보여준다.

예외적으로 포트폴리오 화면에서는 사용자가 체크박스로 명시 선택한 보유 PAPER 포지션만 SELL 할 수 있다. 이 예외는 실제 주문 API가 아니라 기존 `PAPER_TRADING` 주문, 리스크 검증, 포트폴리오 반영, history 저장 흐름을 통과한다. 수동 BUY, 후보 실행, trading-flow 실행 버튼은 계속 제공하지 않는다.

자동 실행은 scheduler가 담당한다. 후보 BUY 실행은 `CandidateExecutionService`, 보유 포지션 익절/손절 평가는 `PositionExitExecutionService`를 통해 PAPER 흐름만 실행한다. 하위 호환용 legacy trading scheduler는 `TradingFlowService`를 사용하지만 기본값은 꺼져 있다.

웹은 실제 주문 API, `REAL_TRADING`, Upbit 인증 설정을 추가하지 않는다.

## 작업 관리

문서 인덱스는 `docs/project/PROJECT_PLAN.md`에서 시작한다.
완료 기록은 Git history를 source of truth로 보고, `docs/project/PROJECT_HISTORY.md`에는 최근 맥락만 유지한다.

작업 순서는 `docs/project/PROJECT_NEXT_STEPS.md`를 우선한다. 현재 다음 계획은 운용 데이터 축적 후 pullback bounce 파라미터 튜닝이다.
