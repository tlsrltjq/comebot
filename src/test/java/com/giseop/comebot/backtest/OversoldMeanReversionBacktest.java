...
final class OversoldMeanReversionBacktest {

    private final List<CandleSeries> series;
    private final OversoldMeanReversionConfig config;

    OversoldMeanReversionBacktest(
            List<CandleSeries> series,
            OversoldMeanReversionConfig config
    ) {
        this.series = series;
        this.config = config;
    }
    ...
    private Candidate bestCandidate(List<Integer> active, int[] pointer, Map<String, OpenTrade> open) {
        ...
        for (int i : active) {
            ...
            int idx = pointer[i];
            if (open.containsKey(s.market()) || idx - config.lookbackCandles() < 0 || s.close(idx) <= 0) {
                continue;
            }
            double startClose = s.close(idx - config.lookbackCandles());
            if (startClose <= 0) {
                continue;
            }
            double dropPct = (s.close(idx) / startClose - 1.0) * 100.0;
            if (dropPct > -config.minDropPct()) {
                continue;
            }
            double averageClose = averageClose(s, idx - config.lookbackCandles(), idx);
            if (averageClose <= 0) {
                continue;
            }
            double deviationPct = (averageClose / s.close(idx) - 1.0) * 100.0;
            if (deviationPct < config.minDeviationFromAveragePct()) {
                continue;
            }
            if (s.highLowRangePct(idx) > config.maxCurrentRangePct()) {
                continue;
            }
            double score = Math.abs(dropPct) + deviationPct;
    ...
    private double averageClose(CandleSeries s, int fromInclusive, int toExclusive) {
        double sum = 0;
