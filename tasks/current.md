...
1. 실제 PAPER 관찰 운영 설정 적용 전 `candidateReadinessWarnings=[]`가 되도록 외부 설정을 맞춘다.
2. candidate/exit scheduler를 제한적으로 ON 하고, 첫 관찰 로그/히스토리/포트폴리오 스냅샷을 condition-records에 새 파일로 추가한다.
3. PAPER 관찰 시작 후 `.backtest_cache` prune/delete 범위를 정한다.
