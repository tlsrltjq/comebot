...
1. `scripts/run-session-volatility-paper-jpa.sh`로 백엔드를 기동하고 `/api/system/status`, `/api/market-provider/status`를 확인한다.
2. `scripts/resume-paper-auto.sh`로 candidate/exit scheduler를 제한적으로 ON 한다.
3. 첫 관찰 로그/히스토리/포트폴리오 스냅샷을 condition-records에 새 파일로 추가한다.
4. PAPER 관찰 시작 후 `.backtest_cache` prune/delete 범위를 정한다.
