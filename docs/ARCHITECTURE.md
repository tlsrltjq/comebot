...
- 응답에는 최대 주문 금액, 허용 market 목록, 손절/익절 설정, 일일 리스크 설정을 포함한다.
- Risk 상태 조회 API는 설정 변경 기능을 제공하지 않는다.
- Risk Controller에는 리스크 검증이나 주문 실행 로직을 넣지 않는다.

## Daily Risk Validation

- `risk.daily-risk-enabled=false`가 기본값이다.
- 활성화된 경우 `OrderExecutionService`가 페이퍼 실행 게이트웨이를 호출하기 전에 일일 제한을 검증한다.
- 오늘 FILLED 주문 수는 history 저장소에서 조회한다.
- 오늘 실현 손실은 Paper Portfolio의 실현손익 이벤트에서 손실 금액만 합산한다.
