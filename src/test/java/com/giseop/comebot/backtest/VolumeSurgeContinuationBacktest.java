...
final class VolumeSurgeContinuationBacktest {

    private final List<CandleSeries> series;
    private final VolumeSurgeContinuationConfig config;

    VolumeSurgeContinuationBacktest(List<CandleSeries> series, VolumeSurgeContinuationConfig config) {
        this.series = series;
        this.config = config;
    }
    ...
    private Candidate bestCandidate(List<Integer> active, int[] pointer, Map<String, OpenTrade> open) {
        ...
        for (int i : active) {
            ...
            int idx = pointer[i];
            if (open.containsKey(s.market()) || idx - config.averageWindowCandles() < 0 || s.open(idx) <= 0) {
                continue;
            }
            double candleReturnPct = (s.close(idx) / s.open(idx) - 1.0) * 100.0;
            if (candleReturnPct < config.minCandleReturnPct()) {
                continue;
            }
            ...
            double ratio = s.accTradePrice(idx) / avgAmount;
            if (ratio < config.minVolumeRatio()) {
                continue;
            }
            double score = ratio * Math.max(candleReturnPct, 0.01);
