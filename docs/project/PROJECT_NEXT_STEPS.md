...
- 완료: 모바일 viewport에서 포지션을 카드형 리스트로 표시한다.
- 완료: desktop 표는 유지한다.
- 완료: 긴 market명과 큰 KRW 숫자가 겹치지 않도록 고정 폭과 줄바꿈을 정리한다.
- 완료: 포트폴리오 화면 테스트를 모바일 카드 표시 기준으로 보강한다.
...
- 완료: SELECTED / SKIPPED 요약을 추가한다.
- 완료: 주요 제외 사유 TOP 5를 추가한다.
- 완료: 선택 후보만 보기 필터를 추가한다.
- 완료: 보유 포지션 여부를 표시한다.
- 완료: 포지션 보유 여부와 market별 노출 비중 표시를 준비한다.
