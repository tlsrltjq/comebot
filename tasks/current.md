...
- 운영 코드 반영:
  ...
  - 2026-06-17 KST 관찰에서 `FDUSDUSDT` 28건 반복 PAPER BUY 확인. pegged/stable 제외 누락으로 판단해 automation을 중지했고, `FDUSDUSDT` 기본 제외 패치를 적용한다.
...
2. `FDUSDUSDT` 제외 패치 검증 후 Docker app/web을 재빌드·재기동한다.
3. scheduler readiness를 다시 확인한 뒤 Session Volatility PAPER automation 재개 여부를 결정한다.
4. 다음 15:00-21:00 KST(UTC 06-12) 신규 진입 세션의 후보/체결/미체결을 확인한다.
5. 관찰 결과에 따라 Binance `ALL_USDT` 유지, market 축소, 또는 scheduler OFF를 결정한다.
6. 주식 PAPER 확장 계획(`tasks/stock-expansion-plan.md`)에 따라 미국 주식 데이터 provider 후보와 모델링 방식을 결정한다.
7. PAPER 관찰 시작 후 `.backtest_cache` prune/delete 범위를 정한다. 이때 crypto/stock 캐시 분리 계획도 함께 반영한다.
