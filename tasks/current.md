...
- 운영 코드 반영:
  ...
  - 2026-06-17 KST 관찰에서 `FDUSDUSDT` 28건 반복 PAPER BUY 확인. pegged/stable 제외 누락으로 판단해 automation을 중지했고, `FDUSDUSDT` 기본 제외 패치를 적용했다.
  - `FDUSDUSDT` PAPER 포지션은 선택 PAPER SELL로 정리했다. 확정 손익은 `-0.140070035 USDT`.
  - readiness 재확인 후 Session Volatility PAPER automation을 재개했다. 현재 후보 30초/exit 5초/Binance 전용/경고 없음.
  - 2026-06-22 점검에서 `FDUSDUSDT` 재진입은 없었지만, 동일 market 반복 BUY 누적으로 `WLDUSDT`, `REUSDT`, `BNBUSDT`, `AVAXUSDT`, `ASTERUSDT`, `SYNUSDT`가 여러 번 진입한 것을 확인했다. 원인은 `STRATEGY_ENTRY_PREVENT_REENTRY_WITH_POSITION=false` 기본값. automation은 중지했다.
  - Session Volatility 관찰 프로필과 Docker Compose override에 `STRATEGY_ENTRY_PREVENT_REENTRY_WITH_POSITION=true`를 추가하고 readiness 경고에 포함했다.
  - BNB 오염 포지션은 선택 PAPER SELL로 정리했다. 확정 손익은 `-0.5345330816 USDT`.
  - readiness/network 재확인 후 Session Volatility PAPER automation을 재개했다. 현재 Binance open position은 0개다.
...
2. 다음 15:00-21:00 KST(UTC 06-12) 신규 진입 세션에서 같은 market 반복 BUY가 차단되는지 확인한다.
...
## Previous Session Stop Point (2026-06-24)

- Pending limit order duplicate placement guard implemented with atomic `tryPlace(...)` and same-market blocked candidate result logging.
- Operational log retention implemented with `candidate_scan_daily_summary`, 30-day candidate detail retention, and 90-day trading flow history retention defaults.
...
## Previous Session Stop Point (2026-06-25, stock import foundation)

- Stock identity/import foundation completed in commit `6a70240 Add stock candle import foundation`.
- Implemented `MarketAssetClass`, `MarketVenue`, `MarketIdentity`, `CandleInterval`, `StockCandleImportManifest`, `StockCandleRow`, and `StockCandleCsvImporter`.
- Added test-only stock CSV to `CandleSeries` loading through `BacktestSeriesLoader.loadStockSeries(...)`.
