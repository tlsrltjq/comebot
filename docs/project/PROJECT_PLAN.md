...
다음 단계는 MVP2다. MVP1의 market별 최소 리스크 제한은 보류하고, Upbit/Binance, 멀티 전략 profile, spot/futures simulation을 다루는 실험 플랫폼으로 확장한다.

MVP2 초기 구현은 실제 주문 없이 public market data와 PAPER/SIMULATION으로만 진행한다. 첫 단계는 `com.giseop.comebot.mvp2` 패키지와 용어 경계를 만들고 MVP1 자동매매 흐름과 섞이지 않게 분리하는 것이다.
