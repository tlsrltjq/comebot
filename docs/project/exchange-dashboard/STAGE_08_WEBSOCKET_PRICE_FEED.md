...
- `TickerSnapshot`, `PriceSource`, `TickerSnapshotStore`를 추가해 거래소/market별 최신 ticker snapshot을 저장한다.
- `MarketPriceProviderType.SNAPSHOT`과 `SnapshotMarketPriceProvider`를 추가했다.
- `SNAPSHOT` provider는 주문용 freshness 기준 안의 snapshot을 먼저 사용하고, 없거나 stale이면 Upbit/Binance REST fallback provider를 호출한다.
- REST fallback 성공 결과는 `REST_FALLBACK` source snapshot으로 다시 저장한다.
- REST fallback도 실패하면 stale snapshot만으로 주문 가격을 만들지 않고 예외를 반환한다.
- Upbit/Binance WebSocket client를 분리했고, 설정이 꺼져 있으면 연결하지 않는다.
- WebSocket 메시지 parser 테스트는 외부 네트워크 없이 payload 처리와 snapshot 저장만 검증한다.
- `/api/market-provider/status` 응답에 WebSocket enabled 상태와 snapshot count를 추가했다.

검증:

```text
./gradlew test --tests TickerSnapshotStoreTest --tests SnapshotMarketPriceProviderTest --tests MarketPriceProviderSelectionTest --tests MarketProviderStatusControllerTest --tests UpbitTickerWebSocketClientTest --tests BinanceTickerWebSocketClientTest
```
