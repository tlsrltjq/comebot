...
- macOS
  - shell: `zsh`
  - run script: `scripts/run-local-dev.sh`
  - long-running backend script: `scripts/run-paper-jpa.sh`
  - workspace 예시: `/Users/<user>/workspace/comebot`

- Windows
  - shell: `PowerShell` 또는 `cmd`
  - run script: `scripts\run-local-dev.bat`
  - long-running backend script: `scripts\run-paper-jpa.bat`
